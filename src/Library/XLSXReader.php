...
class XLSXReader {
    ...
    protected function parse() {
        ...
        foreach ($relationshipsXML->Relationship as $rel) {
            if ($rel['Type'] == self::SCHEMA_OFFICEDOCUMENT) {
                ...
                foreach ($workbookXML->sheets->sheet as $sheet) {
                    ...
                    $sheets[(string)$r->id] = array('sheetId' => (int)$sheet['sheetId'], 'name' => (string)$sheet['name']);
                }
        ...
        foreach ($sheets as $rid => $info) {
            $this->sheetInfo[$info['name']] = array('sheetId' => $info['sheetId'], 'rid' => $rid, 'path' => $info['path']);
        }
    }
	...
	public function getDataObject($sheetdata){
		$titlekey = array();
		$alldata = array();
		foreach($sheetdata as $kk=>$vv){
			if($kk==0) $titlekey=$vv;
				...
				$rowdata= array();
				foreach($vv as $ki=>$vd){
					if(!empty($titlekey[$ki]))
						$rowdata[strtolower($titlekey[$ki])]=$vv[$ki];
					else
						$rowdata["fld_".$ki]=$vv[$ki];
				}
				if(count($rowdata)>0)
					$alldata[]=$rowdata;
			}
